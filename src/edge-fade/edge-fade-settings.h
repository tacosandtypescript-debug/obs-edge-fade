...
#define EDGE_FADE_SETTING_UNIFORM "edge_fade.uniform"
...
struct edge_fade_settings {
	bool linked;
	int uniform; /* linked width: mirrors what all four sides hold */
	int left;
	int right;
	int top;
	int bottom;
...
/**
 * True when all four sides already hold the same width. Used to seed the single
 * "all borders" slider when the sides happen to agree.
 */
bool edge_fade_settings_sides_equal(const struct edge_fade_settings *settings);

/**
 * Width the single "all borders" slider should show, given the stored uniform
 * width and the four sides. Keeps a previously chosen global value when the
